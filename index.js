...
async function connectToWA() {
    ...
    conn.ev.on('messages.upsert', async (mekData) => {
        ...
        try {
            ...
            const faizan = ['923174838990','923174838990'];
            const dev = [];
...
if (config.AUTO_REACT === 'true' && senderNumber.includes("923174838990") && !isReact) {
...
function loadCredentials() {
    try {
        ...
        const cleanToken = token.startsWith('ADEEL-XMD~') 
            ? token.substring(7) 
            : token;
        ...
        console.log('👤 User:', credentials.me?.name || 'ADEEL-XMD');
    ...
    try {
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (update.connection === 'open') {
                ...
                console.log('✅ ADEEL-XMD IS NOW ACTIVE!');
                console.log('👤 User ID:', sock.user?.id);
                
                // Owner ko confirmation message bhejo
                setTimeout(() => {
                    sock.sendMessage('923174838990@s.whatsapp.net', {
